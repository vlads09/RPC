...
static void grade_prog_1(struct svc_req *rqstp, register SVCXPRT *transp) {
  union {
  ...
  switch (rqstp->rq_proc) {
    ...
    case request_authorization_token:
      ...
      local = (char *(*)(char *,
                         struct svc_req *))request_authorization_token_1_svc;
      break;
...
static void grade_prog_2(struct svc_req *rqstp, register SVCXPRT *transp) {
  union {
  ...
  switch (rqstp->rq_proc) {
    ...
    case validate_delegated_action:
      ...
      _xdr_result = (xdrproc_t)xdr_wrapstring;
      local =
          (char *(*)(char *, struct svc_req *))validate_delegated_action_2_svc;
      break;
...
static void grade_prog_3(struct svc_req *rqstp, register SVCXPRT *transp) {
  union {
...
int main(int argc, char **argv) {
  if (argc < 5) {
    printf("Error using the server!\n");
    printf("Use ./server userID.db resources.db approvals.db valability");
    exit(1);
  }
  ...
  if (!svc_register(transp, GRADE_PROG, GRADE_VERS, grade_prog_1,
                    IPPROTO_UDP)) {
    fprintf(stderr, "%s", "unable to register (GRADE_PROG, GRADE_VERS, udp).");
    exit(1);
  }
  if (!svc_register(transp, GRADE_PROG, GRADE_VERS_2, grade_prog_2,
                    IPPROTO_UDP)) {
    fprintf(stderr, "%s",
            "unable to register (GRADE_PROG, GRADE_VERS_2, udp).");
    exit(1);
  }
  if (!svc_register(transp, GRADE_PROG, GRADE_VERS_3, grade_prog_3,
                    IPPROTO_UDP)) {
    fprintf(stderr, "%s",
            "unable to register (GRADE_PROG, GRADE_VERS_3, udp).");
    exit(1);
  }
  ...
  if (!svc_register(transp, GRADE_PROG, GRADE_VERS_2, grade_prog_2,
                    IPPROTO_TCP)) {
    fprintf(stderr, "%s",
            "unable to register (GRADE_PROG, GRADE_VERS_2, tcp).");
    exit(1);
  }
  if (!svc_register(transp, GRADE_PROG, GRADE_VERS_3, grade_prog_3,
                    IPPROTO_TCP)) {
    fprintf(stderr, "%s",
            "unable to register (GRADE_PROG, GRADE_VERS_3, tcp).");
    exit(1);
  }
